...
def _extractor_args() -> List[str]:
    """Add YouTube TV client extractor args to fix 403 errors"""
    return ["--extractor-args", "youtube:player_client=tv"]


def _common_args() -> List[str]:
    """Common yt-dlp arguments with fixes for YouTube restrictions"""
    args = []
    args.extend(_cookies_args())
    args.extend(_extractor_args())
    args.extend([
        "--add-header", "User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0 Safari/537.36",
        "--geo-bypass"
    ])
    return args
